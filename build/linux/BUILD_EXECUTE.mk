...
CPP_TARGETDIR           := $(APP_ROOT)/$(APP_NAME)/target/Release
CPP_TARGETDIR_DEBUG     := $(APP_ROOT)/$(APP_NAME)/target/Debug
CPP_OBJECT_MIDDLE       := $(APP_ROOT)/$(APP_NAME)/im_$(PLATFORM)_$(PROJECT_NAME)/$(APP_NAME)/$(TARGET_MODULE)/Release
CPP_OBJECT_MIDDLE_DEBUG := $(APP_ROOT)/$(APP_NAME)/im_$(PLATFORM)_$(PROJECT_NAME)/$(APP_NAME)/$(TARGET_MODULE)/Debug
SDK_LIBRARIES           := $(TARGET_ROOT)/$(PLATFORM)/$(PROJECT_NAME)/Release
SDK_LIBRARIES_DEBUG     := $(TARGET_ROOT)/$(PLATFORM)/$(PROJECT_NAME)/Debug
...
$(RELEASE_TARGET): $(CPP_APP_OBJECT) $(C_APP_OBJECT)
	@echo '> Linking: $@'
	@$(MKDIR) $(CPP_TARGETDIR)
	@$(COPY) $(COPY_LIBRARIES) $(CPP_TARGETDIR)/
	@$(CPP) $(LINK_FLAGS) $(TARGET_ARCH) -o $@ $(CPP_APP_OBJECT) $(C_APP_OBJECT) $(LIBS_DIR) $(LIBS)
...
$(DEBUG_TARGET): $(CPP_APP_OBJECT_DEBUG) $(C_APP_OBJECT_DEBUG)
	...
	@$(COPY) $(COPY_LIBRARIES_DEBUG) $(CPP_TARGETDIR_DEBUG)/
	@$(CPP) $(LINK_FLAGS_DEBUG) $(TARGET_ARCH) -o $@ $(CPP_APP_OBJECT_DEBUG) $(C_APP_OBJECT_DEBUG) $(LIBS_DIR_DEBUG) $(LIBS)
