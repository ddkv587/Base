...
////////////////////////////////////////////////////////////////////////
	...
	void* CMemChecker::malloc(size_t size, UINT uiClassID)
	{
	...
	INT CMemChecker::checkPtr(void* p, const CHAR* pcHint)
	{
		if (p == NULL)
		{
	...
	UINT CMemChecker::registClassName(const CHAR* pcClassName)
	{
		if (pcClassName == NULL)
		{
			return 0;
	...
	size_t CMemChecker::getThreadSize( UINT uiIndex )
	{
	...
	BOOLEAN CMemChecker::generatePoolConfig(const String& strFileName, UINT uiIncBytes)
	{
		...
		if (!file.open((const CHAR*)strFileName, FALSE))
		{
			return FALSE;
		}
	...
	void* CMemChecker::hookMalloc(size_t size, UINT uiClassID)
	{
	...
	INT CMemChecker::calcRangeIndex(size_t size)
	{
		if (m_bCompactSizeRange == FALSE)
		{
			if (size <= 4)
	...
	void CMemChecker::logAllocSize(size_t size)
	{
		m_blockStatAll.llAllocTimes++;
		m_blockStatAll.uiCurrentCount++;
	...
	void CMemChecker::logFreedSize(size_t size)
	{
		m_blockStatAll.uiCurrentCount--;
		m_blockStatAll.uiCurrentSize -= size;
